...
//        return ucwords(trim($base));
 // }
	...
	public static function convert_pdf_to_png_cached($pdf_path, $filename = null) {
		...
		try {
			$img = new Imagick();
			$img->setResolution(150, 150); // improve quality
			$img->readImage($pdf_path . '[0]'); // first page only

			// Set white background and flatten (removes transparency)
			...
			[$width, $height] = getimagesize($png_path);
			file_put_contents($meta_path, json_encode(['width' => $width, 'height' => $height]));
			error_log("[KGSweb] Converted PDF -> PNG (white bg): {$width}x{$height}, saved to $png_path");
 ...
 */	
		public static function register_wp_attachment($file_path, $mime_type = '') {
		$filetype = wp_check_filetype(basename($file_path), null);

		$attachment = [
			'guid'           => wp_upload_dir()['url'] . '/' . basename($file_path),
			'post_mime_type' => $mime_type ?: $filetype['type'],
	 ...
	 * Example output: "Root > Projects > 2025 > September"
	 * Returns an array of ['name' => folder name, 'id' => folder ID]
	 */
	/* public static function get_folder_breadcrumb($folder_id) {
		error_log("Real breadcrumb handler START");
		if (!$folder_id) return [];

		// Check if cached
		$transient_key = 'kgsweb_breadcrumb_' . $folder_id;
		$cached = get_transient($transient_key);
		if ($cached !== false) return $cached;
		
		error_log("Breadcrumb: attempting to get Drive service");
		$integration = KGSweb_Google_Integration::init();
		$service = $integration->get_drive_service();
		if (!$service) {
			error_log("Breadcrumb: Drive service not available");
			return [];
		}
		...
		try {
			...
			while ($currentId) {
				$folder = $service->files->get($currentId, ['fields' => 'id, name, parents']);
				array_unshift($parts, ['name' => $folder->name, 'id' => $folder->id]);
			...
			error_log("Breadcrumb exception: " . $e->getMessage());
			return [['name' => $folder_id, 'id' => $folder_id]];
		}

		// Cache for 12 hours
		set_transient($transient_key, $parts, 12 * HOUR_IN_SECONDS);
