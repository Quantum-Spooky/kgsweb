...
class KGSweb_Google_Integration {
	...
	public static function get_settings(): array {
		...
		$settings['public_docs_root_id']    = get_option('kgsweb_root_folder_id', ''); // Keep raw
		$settings['upload_root_id']         = get_option('kgsweb_upload_root_folder_id', ''); // Keep raw
		$settings['menu_breakfast_folder_id'] = get_option('kgsweb_breakfast_folder_id', ''); // Keep raw
		$settings['menu_lunch_folder_id']   = get_option('kgsweb_lunch_folder_id', ''); // Keep raw
	...
	public static function get_latest_file_from_folder(string $folder_id): ?array {
		if (empty($folder_id)) return null;
			...
			$walker = function($nodes) use (&$walker, &$latest) {
				foreach ((array)$nodes as $n) {
					...
					if (($n['type'] ?? '') === 'file') {
						...
						if (empty($latest) || strcmp($file['modifiedTime'], $latest['modifiedTime']) > 0) {
							$latest = $file;
						}
					}
		...
		if (!$client instanceof Client) {
			error_log("KGSWEB: get_latest_file_from_folder - google client not available");
			return null;
		}
		...
		try {
			...
			foreach ($files as $f) {
				...
				if (in_array($meta['mimeType'], ['application/vnd.google-apps.document', 'text/plain'], true)) {
					$preferred = $meta;
					break;
				}
				// keep first as fallback
				if ($preferred === null) $preferred = $meta;
			}
			...
			error_log('KGSWEB: get_latest_file_from_folder - Drive API error: ' . $e->getMessage());
			return null;
		}
	}
	...
	public function cron_refresh_all_caches(): void {
		...
		if ($calendar_id) {
			KGSweb_Google_Upcoming_Events::refresh_cache_cron(); // internal method will call refresh_events_cache
		...
		if ($slides_file) KGSweb_Google_Slides::refresh_cache($slides_file);

		// --- Refresh Sheets ---
		$sheets_file = self::get_settings()['sheets_file_id'] ?? '';
		if ($sheets_file) KGSweb_Google_Sheets::refresh_cache($sheets_file, 'A1:Z100');
...
public function enqueue_frontend(): void {
    ...
    $frontend_js = ['helpers','format','documents','cache','ticker','calendar','menus','upload','sheets'];
    ...
    foreach ($frontend_js as $mod) {
        ...
        if (file_exists($path)) {
            wp_enqueue_script(
                ...
                ['jquery'],
                filemtime($path), // filemtime ensures cache-busting
                true
            );
        }
    }
