...
class KGSweb_Google_Drive_Docs
{
    ...
    {
        return KGSweb_Google_Integration::get_settings()[
    ...
    public static function rebuild_documents_tree_cache($root)
    {
        ...
        KGSweb_Google_Integration::set_transient(
            "kgsweb_cache_documents_" . $root,
            $tree,
            HOUR_IN_SECONDS
        );
        update_option(
            "kgsweb_cache_last_refresh_documents_" . $root,
    ...
    public static function get_documents_tree_payload($folder_id = "")
    {
        ...
        if (empty($tree)) {
            return new WP_Error(
                "no_docs",
                __("No documents available.", "kgsweb"),
                ["status" => 404]
            );
        }
        ...
        while (!empty($queue)) {
            ...
            foreach ($items as $item) {
                ...
                if ($node["type"] === "file") {
                    ...
                    $node["icon"] = KGSweb_Google_Helpers::icon_for_mime_or_ext(
                        $item["mimeType"],
                        $ext
                    );
                    $children[] = $node;
    ...
    public static function list_drive_children(
        $folder_id,
        $sort_by = "alpha-asc"
    ) {
        ...
        try {
            ...
            error_log(
                "KGSWEB: Failed to list files in folder {$folder_id} - " .
                    $e->getMessage()
            );
            return [];
        }
    ...
    private function get_docs_service()
    {
        ...
        try {
            ...
            if (!$client instanceof Client) {
                error_log(
                    "KGSWEB: get_docs_service - google client not available"
                );
                return null;
            }
            ...
            error_log(
                "KGSWEB: Failed to initialize Docs service - " .
                    $e->getMessage()
            );
            return null;
        }
    ...
    public function get_file_contents($file_id, $mime_type = null)
    {
        if (!$this->client) {
            error_log(
                "KGSWEB: get_file_contents called but client not initialized!"
            );
            return "";
        }
        ...
        try {
            ...
            if (empty($this->service)) {
                ...
                error_log(
                    "KGSWEB: get_file_contents - Drive service initialized."
                );
            }
            ...
            if (!$mime_type) {
                ...
                error_log(
                    "KGSWEB: get_file_contents - using provided MIME {$mime_type} for {$file_id}"
                );
            }
            ...
            if ($mime_type === "application/vnd.google-apps.document") {
                error_log(
                    "KGSWEB: get_file_contents - attempting Docs API extraction for {$file_id}"
                );
                ...
                try {
                    ...
                    if ($docsService) {
                        ...
                        if (
                            ...
                            $content = KGSweb_Google_Helpers::extract_text_from_doc(
                                $doc
                            );
                            error_log(
                                "KGSWEB: get_file_contents - Docs API extraction returned " .
                                    strlen($content) .
                                    " chars."
                            );
                            return $content;
                            ...
                            error_log(
                                "KGSWEB: get_file_contents - Docs API returned empty body, falling back to Drive export."
                            );
                        }
                    } else {
                        error_log(
                            "KGSWEB: get_file_contents - Docs API unavailable, falling back to Drive export."
                        );
                    }
                } catch (Exception $e) {
                    error_log(
                        "KGSWEB: get_file_contents - Docs API exception: " .
                            $e->getMessage()
                    );
                }
            ...
            if ($mime_type === "text/plain") {
                ...
                error_log(
                    "KGSWEB: get_file_contents - downloaded " .
                        strlen($content) .
                        " bytes for TXT file."
                );
                return $content;
            ...
            error_log(
                "KGSWEB: get_file_contents - unsupported MIME {$mime_type}, skipping."
            );
            return "";
        } catch (Exception $e) {
            error_log(
                "KGSWEB: get_file_contents - EXCEPTION for {$file_id}: " .
                    $e->getMessage()
            );
            return "";
        }
    ...
    public static function get_cached_folders(string $root_id): array
    {
        ...
        $build_tree = function (string $folder_id, array $path = []) use (
            &$build_tree
        ) {
            $items = self::list_drive_children($folder_id);
            $folders = [];
    ...
    public static function cache_upload_folders(string $root_id): void
    {
        ...
        update_option(
            "kgsweb_cache_last_refresh_upload_folders_" . $root_id,
            current_time("timestamp")
        );
    }
}
