...
/**
 * Universal Person Resolver
 * Finds a person in the cache by name or email.
 */
function get_person($query): ?array {
    ...
    foreach ($sources as $src) {
        $path = ROOT_PATH . "kgs-cache/google/people_{$src}.json";
        ...
        foreach ($people as $p) {
            ...
            if ($email === $query) return $p;
            
            // 2. Direct match on Name
            if ($name === $query) return $p;
            
            // 3. Fuzzy match: Does the query appear inside the name? 
            if (str_contains($name, $query)) return $p;
        }
    }
    return null;
}
...
/**
 * Resolves a Leadership Role to a full Person object with an overridden name.
 */
function get_featured_profile(string $roleKey): ?array {
    $rolesPath = ROOT_PATH . "kgs-cache/google/leadership_map.json";
    $roles = file_exists($rolesPath) ? json_decode(file_get_contents($rolesPath), true) : [];
    
    if (!isset($roles[$roleKey])) return null;
    $role = $roles[$roleKey];

    // 1. Handle "MANUAL:" entries (for people not in the Staff Directory)
    if (str_starts_with($role['person_slug'], 'MANUAL:')) {
        return [
    ...
    // 2. Fetch the base person data from our existing people caches
    // (We'll use a modified version of the get_person helper that looks for slugs)
    $person = get_person_by_slug($role['person_slug']);
    
    if ($person) {
        // APPLY THE OVERRIDE
        $person['name'] = !empty($role['display_name']) ? $role['display_name'] : $person['name'];
        return $person;
    }
