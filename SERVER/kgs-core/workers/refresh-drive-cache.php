...
try {
		...
		try {
				...
				if ($response) {
					...
					foreach ($response as $r) {
						$k = trim($r[0] ?? ''); // Col A (Key)
						$l = trim($r[1] ?? ''); // Col B (Friendly Name / Label)
						if ($k && $k !== 'none' && $k !== 'Key') {
							$wReg[$k] = [
								'label'     => $l, 
								'component' => trim($r[2] ?? ''), 
								'source'    => trim($r[3] ?? ''),
								'parameter' => trim($r[4] ?? 'id')
							];
							$wLkp[$l] = $k;
						}
					}
	...
	*/
		...
		if ($peopleId) {
			try {
				$response = $service->spreadsheets_values->get($peopleId, 'People!A:K')->getValues();
			...
			/*
    |--------------------------------------------------------------------------
    | TASK: SYNC LEADERSHIP REGISTRY
    |--------------------------------------------------------------------------
    */
    echo " - Syncing Leadership Roles... ";
    try {
        $response = $service->spreadsheets_values->get($peopleId, 'Leadership Registry!A:D');
        $rows = $response->getValues();
        $rolesMap = [];
        if ($rows) {
            array_shift($rows); // Skip labels
            foreach ($rows as $row) {
                $key = trim($row[0] ?? '');
                if (empty($key)) continue;

                $rolesMap[$key] = [
                    'person_slug' => trim($row[1] ?? ''), // e.g. "heitmeyer-harry"
                    'display_name' => trim($row[2] ?? ''), // e.g. "Coach Heitmeyer"
                    'category'    => trim($row[3] ?? '')
                ];
            }
            file_put_contents(ROOT_PATH . "kgs-cache/google/leadership_map.json", json_encode($rolesMap, JSON_PRETTY_PRINT));
            echo "DONE\n";
        }
    } catch (Exception $e) { echo "FAILED "; }
	...
	*/
		if (!empty($configMap['live_feed_sheet_id'])) {
			...
			try {
				$response = $service->spreadsheets_values->get($configMap['live_feed_sheet_id'], 'Posts!A:F')->getValues();
				if ($response) {
...
/*
|--------------------------------------------------------------------------
| TASK 12: SYNC SITEMAP (Pages Tab)
|--------------------------------------------------------------------------
*/
echo " - Task 12: Sitemap... ";
try {
    // UPDATED: Range A:J to include Template, Layout, and Prefix
	$response = $service->spreadsheets_values->get($sheetId, 'Pages!A:J');
	$rows = $response->getValues();
    $sitemap = [];
    if ($rows) {
        array_shift($rows); 
        foreach ($rows as $row) {
            ...
            $sitemap[$route] = [
                'title'    => trim($row[1] ?? ''),
                'doc_id'   => trim($row[2] ?? ''), // Column C: About
                'data_id'  => trim($row[5] ?? ''), // Column F: Data
                'cal_id'   => trim($row[6] ?? ''), // Column G: Calendar
                'template' => trim($row[7] ?? 'Standard'), // Column H
                'layout'   => trim($row[8] ?? 'default'),  // Column I
                'prefix'   => trim($row[9] ?? '')          // Column J
            ];
        }
        file_put_contents($baseDir . "sitemap.json", json_encode($sitemap, JSON_PRETTY_PRINT));
        echo "DONE\n";
    }
} catch (Exception $e) { echo "FAILED "; }
