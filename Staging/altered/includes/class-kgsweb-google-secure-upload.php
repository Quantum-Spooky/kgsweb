...
class KGSweb_Google_Secure_Upload {
    ...
    public static function handle_upload_rest(WP_REST_Request $req) {
        ...
        if ($auth_mode === 'password') {
            ...
            if (!$authorized) self::register_failed_attempt($key);
        } else {
            $authorized = self::verify_google_group(); // TODO
            if (!$authorized) self::register_failed_attempt($key);
        }
    ...
    private static function upload_to_wordpress($folder_id, $file) {
        ...
        $subpath = KGSweb_Google_Drive_Docs::folder_path_from_id($folder_id);
    ...
    private static function verify_password($password) {
        ...
        return hash_equals($hash, hash($algo, $password));
    }
    ...
    /*******************************
     * CRON / Upload tree cache refresh
     *******************************/
