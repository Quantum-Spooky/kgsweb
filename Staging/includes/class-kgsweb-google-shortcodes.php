...
class KGSweb_Google_Shortcodes {
	...
	public static function register_assets() {
		...
		// Enqueue helpers first (needed by other scripts)
		wp_register_script(
			'kgsweb-helpers',
		...
		wp_register_script(
			...
			['kgsweb-helpers'], // ensure helpers loaded first
			$ver,
			true
		);
	...
	public static function enqueue_documents_assets() {
		wp_enqueue_style('kgsweb-style');
		wp_enqueue_script('kgsweb-folders'); // will automatically enqueue 'kgsweb-helpers' first
	}	  
	...
	public static function documents($atts = []) {
		...
		$atts = shortcode_atts([
			...
			'sort_mode'     => 'alpha-asc',  // alpha-asc / alpha-desc / date-asc / date-desc
			'folders_first' => 'true',       // 'true' / 'false'
		...
		$folders_first = filter_var($atts['folders_first'], FILTER_VALIDATE_BOOLEAN) ? 'true' : 'false';
		$sort_mode = in_array(strtolower($atts['sort_mode']), ['alpha-asc','alpha-desc','date-asc','date-desc']) ? strtolower($atts['sort_mode']) : 'alpha-asc';

		// Data attributes for JS
		$data_attrs = sprintf(
			' data-root-id="%s" data-sort-mode="%s" data-folders-first="%s"',
			esc_attr($root),
			esc_attr($sort_mode),
			esc_attr($folders_first)
		);
